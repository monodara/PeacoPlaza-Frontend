...
export default function CategoryCard({ category }: { category: CategoryReadDto }) {
  ...
  return (
    <div className="relative w-full max-w-sm mx-auto rounded-md shadow-md overflow-hidden bg-green-400">
      ...
      <div className="pt-6">
        <h5 className="mb-2 text-xl font-medium leading-tight text-gray-100" style={{ color: primaryColor }}>
          {category.name}
        </h5>
      </div>

      <div className="pb-6">
        <Link to="/products" onClick={()=>dispatch(setCategoryBy(category.id))}>
          <div className="inline-block cursor-pointer rounded text-gray-100 font-normal leading-normal text-primary transition duration-150 ease-in-out hover:text-primary-600 focus:text-primary-600 focus:outline-none focus:ring-0 active:text-primary-700" style={{ color: primaryColor }}>
            Shop now
          </div>
        </Link>
      </div>
